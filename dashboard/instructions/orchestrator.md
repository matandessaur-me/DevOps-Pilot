...
4. **Task completions are announced automatically.** When a worker finishes, a one-line hint is injected into your terminal: `[TASK DONE <id>] <state> (<cli>). Fetch full result: GET /api/orchestrator/task?id=<id>`. The worker's output is NOT inlined in the hint -- PTY pastes truncate and large outputs bloat your context. Fetch the full result with `curl -s "http://127.0.0.1:3800/api/orchestrator/task?id=<id>"` (bash) or `Invoke-RestMethod "http://127.0.0.1:3800/api/orchestrator/task?id=<id>"` (PowerShell). You do NOT need to poll. Continue your own work and fetch results as the hints arrive. If you need to check older results or the inbox manually: `curl -s "http://127.0.0.1:3800/api/orchestrator/inbox?termId=main&unread=1"`
